...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = ((F.normalize(q, dim=-1) + self.query_embedding) * F.softplus(self.temperature)) @ F.normalize(k, dim=-1).transpose(-2, -1)
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        v = v.permute(0, 2, 1, 3).view(B, N, -1).contiguous()
        x = v + x
        return x
...
def record_unique_tensor_shapes_to_file(x0, x1, x2, shape_set, filename):
    # 获取每个张量的形状，并将其转换为元组
    shape_x0 = tuple(x0.shape)
    shape_x1 = tuple(x1.shape)
    shape_x2 = tuple(x2.shape)

    # 将形状添加到集合中（集合会自动去重）
    shape_set.add(shape_x0)
    shape_set.add(shape_x1)
    shape_set.add(shape_x2)
...
class MultiModalViT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if self.is_multi_modal:
            ...
            attention_weights = torch.nn.Parameter(torch.rand(3, requires_grad=True))
            attention_weights = torch.nn.functional.softmax(attention_weights, dim=0)
            weighted_x0 = attention_weights[0] * x0
            weighted_x1 = attention_weights[1] * x1
            weighted_x2 = attention_weights[2] * x2

            #
            # # 进行加权求和
            x = torch.cat([weighted_x0, weighted_x1, weighted_x2], dim=1)
