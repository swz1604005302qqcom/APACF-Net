...
def do_valid_test( net, test_loader, criterion ):
    ...
    tpr, fpr, acc = calculate_accuracy(0.5, probs[:,1], labels)
    acer,_,_,_,_,_,_ = ACER(0.5, probs[:, 1], labels)
    _,apcer,_,_,_,_,_= ACER(0.5, probs[:, 1], labels)
    _,_,npcer,_,_,_,_= ACER(0.5, probs[:, 1], labels)

    valid_loss = np.array([
        apcer, acer, npcer, correct
    ])
...
#         valid_num += len(input)
#         probs.append(prob.data.cpu().numpy())
#         labels.append(truth.data.cpu().numpy())  # 收集真实标签
#
#     probs = np.concatenate(probs)
#     labels = np.concatenate(labels)
#
...
#
#     # 返回指标和输出
#     infer_loss = np.array([apcer, acer, npcer, acc])
#     return infer_loss, [probs[:, 1], labels]
