...
def run_train(config):
    ...
    optimizer = optim.SGD(filter(lambda p: p.requires_grad, net.parameters()),
                          lr=0.05, momentum=0.9, weight_decay=0.0005)
    ...
    for cycle_index in range(config.cycle_num):
        ...
        for epoch in range(0, config.cycle_inter):
            ...
            for input, truth in train_loader:
                ...
                loss_Intra = IntraClass_Loss(logit, truth)
                loss_Intra = loss_Intra.cuda()
                loss_Intra.requires_grad_(True)
                lossAll = loss + loss_Intra
            ...
            asterisk = ' '
            log.write(model_name+' Cycle %d: %0.4f %5.1f %6.1f | %0.6f  %0.6f  %0.6f %s  | %0.6f  %0.6f |%s \n' % (
...
def run_test(config, dir):
    ...
    # print('do_valid_test done')

    out = infer_test(net, test_loader)
    # infer_loss, out = infer_test(net, test_loader)
    print('%0.6f  %0.6f  %0.3f  (%0.3f) \n' % (infer_loss[0], infer_loss[1], infer_loss[2], infer_loss[3]))
    print('done')
...
def main(config):
    ...
    if config.mode == 'run test':
        config.pretrained_model = r'/autodl-fs/data/216/ViT_Fusion_CeFA_gai/Models_other_net/FaceBagNetFusion_fusion_96/checkpoint/global_min_acer_model.pth'
        run_test(config, dir='global_test_36_TTA')

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--train_fold_index', type=int, default = -1)
    parser.add_argument('--model', type=str, default='ViTFusion')
    ...
    parser.add_argument('--mode', type=str, default='train', choices=['train','run test'])
    parser.add_argument('--pretrained_model', type=str, default=None)
    parser.add_argument('--save_dir', type=str, default='./Models_val')
